export default {
  ...
  async loadSearchingMovies({ commit }, filters) {
    try {
      ...
      const movies = await this.$services.movies.search(id, name, genres).then(res => res.data);
  ...
  async loadMovieShowList({ commit }, id) {
    try {
      const moviesShowList = await this.$services.movies.showListById(id).then(res => Object.entries(res.data)[0][1]);
  ...
  async loadMoviePlaces({ commit }, data) {
    try {
      ...
      const moviePlaces = await this.$services.book.getPlaces(id, daytime, showdate).then(res => res.data);
  ...
  async BookMovie({ commit }, payload) {
    try {
      ...
      const moviePlaces = await this.$services.book.bookPlace(movie_id, row, seat, showdate, daytime).then(res => res.data);
